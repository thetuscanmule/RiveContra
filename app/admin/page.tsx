    ...
    <div className="min-h-screen bg-gray-50">
      ...
      {tab === 'ui' && <>
        ...
        <section>
          ...
          <h3 className="mt-8 mb-3 text-xs font-semibold uppercase tracking-widest text-gray-400">Gradient grain</h3>
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <FieldLabel>Grain opacity</FieldLabel>
                  <p className="text-xs text-gray-400">Procedural noise overlay on the background gradient. 0 disables it entirely.</p>
                </div>
                <span className="font-mono text-xs text-gray-500 shrink-0 ml-6">{settings.grain.toFixed(2)}</span>
              </div>
              <input
        ...
        <section>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
            ...
            {/* Option button block offset */}
            <div className="px-5 py-4">
              <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-gray-400">Option button block offset (% vh)</p>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <FieldLabel>Desktop</FieldLabel>
                    <span className="font-mono text-xs text-gray-500">{settings.layout.optionBlockOffset > 0 ? '+' : ''}{settings.layout.optionBlockOffset}%</span>
                  </div>
                  <input type="range" min={-30} max={30} step={1}
                    value={settings.layout.optionBlockOffset}
                    onChange={e => setSettings(s => ({ ...s, layout: { ...s.layout, optionBlockOffset: Number(e.target.value) } }))}
                    className="w-full accent-gray-700" />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <FieldLabel>Mobile</FieldLabel>
                    <span className="font-mono text-xs text-gray-500">{settings.layout.optionBlockOffsetMobile > 0 ? '+' : ''}{settings.layout.optionBlockOffsetMobile}%</span>
                  </div>
                  <input type="range" min={-30} max={30} step={1}
                    value={settings.layout.optionBlockOffsetMobile}
                    onChange={e => setSettings(s => ({ ...s, layout: { ...s.layout, optionBlockOffsetMobile: Number(e.target.value) } }))}
                    className="w-full accent-gray-700" />
                </div>
              </div>
            </div>
        ...
        <section>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Hover scale</FieldLabel>
                <p className="text-xs text-gray-400">How much every hex button grows on hover. 1 = no scaling.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" min={1} max={1.5} step={0.01}
                  value={settings.buttonHoverScale}
                  onChange={e => setSettings(s => ({ ...s, buttonHoverScale: Number(e.target.value) }))}
                  className="w-20 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
        ...
        <section>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
            ...
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <FieldLabel>Dice scene offset</FieldLabel>
                  <p className="text-xs text-gray-400">Vertical offset applied instantly when scene 2 (dice) is active. Negative moves up.</p>
                </div>
                <span className="font-mono text-xs text-gray-500 shrink-0 ml-6">{settings.riveDiceOffset > 0 ? '+' : ''}{settings.riveDiceOffset}vh</span>
              </div>
              <input
                type="range" min={-20} max={20} step={0.5}
                value={settings.riveDiceOffset}
            ...
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <FieldLabel>Dice scene scale</FieldLabel>
                  <p className="text-xs text-gray-400">Scale applied to the Rive canvas when the dice scene is active.</p>
                </div>
                <span className="font-mono text-xs text-gray-500 shrink-0 ml-6">{settings.riveDiceScale.toFixed(2)}×</span>
              </div>
              <input
                type="range" min={0.3} max={2} step={0.01}
                value={settings.riveDiceScale}
                onChange={e => setSettings(s => ({ ...s, riveDiceScale: Number(e.target.value) }))}
                className="w-full accent-gray-700"
              />
            </div>
        ...
        <section>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
            ...
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <FieldLabel>Logo gap</FieldLabel>
                  <p className="text-xs text-gray-400">Vertical space between the logo and the Enter button. Negative pulls them closer.</p>
                </div>
                <span className="font-mono text-xs text-gray-500 shrink-0 ml-6">{settings.startScreen.logoGap}px</span>
              </div>
              <input
                type="range" min={-40} max={40} step={1}
                value={settings.startScreen.logoGap}
                onChange={e => setSettings(s => ({ ...s, startScreen: { ...s.startScreen, logoGap: Number(e.target.value) } }))}
                className="w-full accent-gray-700"
              />
            </div>
        ...
        <section>
          ...
          <div className="space-y-3">
            ...
            <AudioPhaseCard
              phaseKey="ui-hover"
              label="Button hover"
              note="Plays once when the cursor enters a HexButton"
              clip={settings.audio.ui?.hover ?? { src: '', volume: 1.0, loop: false }}
              onChange={clip => setSettings(s => ({ ...s, audio: { ...s.audio, ui: { ...s.audio.ui, hover: clip } } }))}
            />
            <div className="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-5 py-4">
              ...
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  ...
                  value={settings.optionButtonHoverVolumeMultiplier}
                  onChange={e => setSettings(s => ({ ...s, optionButtonHoverVolumeMultiplier: Number(e.target.value) }))}
                  className="w-20 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">×</span>
              </div>
        ...
        <section>
          ...
          <div className="rounded-lg border border-gray-200 bg-white divide-y divide-gray-100">
            ...
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <FieldLabel>Jaw default pose</FieldLabel>
                  <p className="text-xs text-gray-400">Jaw open value (0–1) used when idle — between lines of dialogue and before speech starts.</p>
                </div>
                <span className="font-mono text-xs text-gray-500 shrink-0 ml-6">{settings.jawDefault.toFixed(2)}</span>
              </div>
              <input
                type="range" min={0} max={1} step={0.01}
                value={settings.jawDefault}
                onChange={e => setSettings(s => ({ ...s, jawDefault: Number(e.target.value) }))}
                className="w-full accent-gray-700"
              />
            </div>

            {/* Jaw default pose extended */}
            <div className="px-5 py-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <FieldLabel>Jaw default pose extended</FieldLabel>
                  <p className="text-xs text-gray-400">Second jaw value the idle animation oscillates toward and back from.</p>
                </div>
                <span className="font-mono text-xs text-gray-500 shrink-0 ml-6">{settings.jawDefaultExtended.toFixed(2)}</span>
              </div>
              <input
                type="range" min={0} max={1} step={0.01}
                value={settings.jawDefaultExtended}
                onChange={e => setSettings(s => ({ ...s, jawDefaultExtended: Number(e.target.value) }))}
                className="w-full accent-gray-700"
              />
            </div>
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Jaw idle anim speed</FieldLabel>
                <p className="text-xs text-gray-400">Duration of one full idle oscillation cycle between the two jaw poses.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" min={100} step={100}
                  value={settings.jawIdleSpeed}
                  onChange={e => setSettings(s => ({ ...s, jawIdleSpeed: Number(e.target.value) }))}
                  className="w-24 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">ms</span>
              </div>
            ...
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Jaw return duration</FieldLabel>
                <p className="text-xs text-gray-400">How long the jaw takes to lerp back to the default pose after each line of dialogue.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" min={0} step={50}
                  value={settings.jawReturnDuration}
                  onChange={e => setSettings(s => ({ ...s, jawReturnDuration: Number(e.target.value) }))}
                  className="w-24 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">ms</span>
              </div>
            ...
            {/* Results UI reveal delay */}
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Results UI reveal delay</FieldLabel>
                <p className="text-xs text-gray-400">Delay after entering the results screen state before the score and buttons fade in.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" min={0} step={100}
                  value={settings.pauseResultsReveal}
                  onChange={e => setSettings(s => ({ ...s, pauseResultsReveal: Number(e.target.value) }))}
                  className="w-24 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">ms</span>
              </div>
            ...
            {/* Results crossfade — Rive fade-out duration */}
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Results crossfade — Rive fade-out</FieldLabel>
                <p className="text-xs text-gray-400">How long the Rive canvas takes to fade out as the results screen appears.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" min={0} step={50}
                  value={settings.resultsCrossfade.riveFadeDuration}
                  onChange={e => setSettings(s => ({ ...s, resultsCrossfade: { ...s.resultsCrossfade, riveFadeDuration: Number(e.target.value) } }))}
                  className="w-24 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">ms</span>
              </div>
            ...
            {/* Results crossfade — UI fade-in duration */}
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Results crossfade — UI fade-in</FieldLabel>
                <p className="text-xs text-gray-400">How long the score and buttons take to fade in once revealed.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" min={0} step={50}
                  value={settings.resultsCrossfade.uiFadeDuration}
                  onChange={e => setSettings(s => ({ ...s, resultsCrossfade: { ...s.resultsCrossfade, uiFadeDuration: Number(e.target.value) } }))}
                  className="w-24 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">ms</span>
              </div>
            ...
            {/* Results crossfade — offset */}
            <div className="flex items-center justify-between px-5 py-4">
              <div>
                <FieldLabel>Results crossfade — offset</FieldLabel>
                <p className="text-xs text-gray-400">Shifts when the Rive fade-out starts, relative to the reveal delay above. Negative starts it earlier, positive later.</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0 ml-6">
                <input
                  type="number" step={50}
                  value={settings.resultsCrossfade.offset}
                  onChange={e => setSettings(s => ({ ...s, resultsCrossfade: { ...s.resultsCrossfade, offset: Number(e.target.value) } }))}
                  className="w-24 rounded border border-gray-200 px-2 py-1.5 text-right font-mono text-sm text-gray-700 focus:border-gray-400 focus:outline-none"
                />
                <span className="text-xs text-gray-400">ms</span>
              </div>
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white px-6 py-5">
      ...
      <div className="relative">
        ...
        <TLEvent label="Results screen state entered" />
        <TLPause label="pauseResultsReveal — score and buttons fade in" value={settings.pauseResultsReveal} maxMs={maxMs} input={num('pauseResultsReveal')} />

        <TLEvent label="Results UI visible" />
      </div>
    </div>
  );
}
